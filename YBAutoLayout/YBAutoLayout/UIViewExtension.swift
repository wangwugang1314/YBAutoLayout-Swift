...
extension UIView {
    ...
    ///   - target: 目标
    ///   - action: 方法
    func yb_addEvent_tap(target: Any?, action: Selector?) {
    ...
    ///   - target: 目标
    ///   - action: 方法
    func yb_addEvent_pinch(scale: CGFloat = 1, target: Any?, action: Selector?) {
        let event = UIPinchGestureRecognizer(target: target, action: action)
        event.scale = scale
    ...
    ///   - target: 目标
    ///   - action: 方法
    func yb_addEvent_swipe(direction: UISwipeGestureRecognizerDirection, target: Any?, action: Selector?) {
        let event = UISwipeGestureRecognizer(target: target, action: action)
    ...
    ///   - target: 目标
    ///   - action: 方法
    func yb_addEvent_longPress(time: CFTimeInterval, target: Any?, action: Selector?) {
        let event = UILongPressGestureRecognizer(target: target, action: action)
        event.minimumPressDuration = time
        addGestureRecognizer(event)
    }
}
