...
class YBController_13: YBBaseController {
    ...
    override func viewDidLoad() {
        ...
        shwoTextArr = [YBCellModel(title: "1. 如果是label的话不能使用yb_alignment布局，必须指定宽度（目前我也不是太清楚原因，约束的这样约束的时候会出问题）"),
                       YBCellModel(title: "2. 可以设定内容大小与底部的间距")]
    ...
    override func prepareUI() {
        ...
        let num = Int.yb_random(between: 10, and: 100)
        for _ in 0..<num {
            dataStr += str
        }
        let label = UILabel()
        ...
        scrollView.yb_in(view: label, duration: .centerTop, size: yb_layoutSize(width: 200))
        scrollView.yb_setupAutoContentSize(bottomView: label, bottomMargin: 10)
    }
